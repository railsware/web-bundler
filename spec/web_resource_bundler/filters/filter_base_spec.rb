...
describe WebResourceBundler::Filters::BaseFilter do
  describe "#apply" do
    context "exception raised during block invocation" do
      ...
      it "logs error" do
        @logger.should_receive(:error).with("WebResourceBundler::Filters::BaseFilter: #{@error_text}")
        Filters::BaseFilter.new(@settings, @logger).apply() { raise @exception }
      end
      ...
      context "clenaup called" do
        ...
        class FilterChild < Filters::BaseFilter
