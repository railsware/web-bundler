  ...
  describe "#apply" do
    context "block was bundled" do
      it "encodes images in bundles by creating two files (for IE and others) if block_data without condition" do
        ...
        block_data.result_files.include?(File.join(@settings.cache_dir, @file_prefix + block_data.css.bundle_filename(@settings))).should be_true
        block_data.result_files.include?(File.join(@settings.cache_dir, @ie_file_prefix + block_data.css.bundle_filename(@settings))).should be_true
      end
    ...
    context "block wasn't bundled" do
      it "encodes separatly all css files" do
        ...
        block_data.result_files.include?(File.join(@settings.cache_dir, @file_prefix + File.basename('sample.css'))).should be_true
        #foo.css has no images in there, so it shouldn't be encoded
        block_data.result_files.include?('/foo.css').should be_true
      end
    end
  end
end
